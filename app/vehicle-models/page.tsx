...
export default function VehicleModelManagementPage() {
  ...
  const handleDelete = async (modelId: string) => {
    if (!confirm('Are you sure you want to delete this vehicle model? This action cannot be undone.')) return;
    try {
      await vehicleModelService.delete(modelId);
      toast.success('Vehicle model deleted successfully');
      fetchModels();
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to delete vehicle model');
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage vehicle models (create models before creating variants)</p>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="rounded-2xl max-w-2xl">
              <form onSubmit={handleSubmit}>
                <DialogHeader>
                  <DialogTitle>
                    {editingModel ? 'Edit Vehicle Model' : 'Create New Vehicle Model'}
                  </DialogTitle>
                  <DialogDescription>
                    {editingModel ? 'Update vehicle model information' : 'Add a new vehicle model to the catalog'}
                  </DialogDescription>
                </DialogHeader>
